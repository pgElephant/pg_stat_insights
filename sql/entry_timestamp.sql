...
SELECT stats_since >= :'ref_ts', count(*) FROM pg_stat_insights
...
SELECT pg_stat_insights_reset(0, 0, queryid, true) AS minmax_reset_ts
FROM pg_stat_insights
...
FROM pg_stat_insights_info;

-- Perform common min/max reset
SELECT pg_stat_insights_reset(0, 0, 0, true) AS minmax_reset_ts \gset
