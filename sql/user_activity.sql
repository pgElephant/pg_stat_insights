...
SET pg_stat_insights.track_utility = TRUE;
...
RESET ROLE;
SELECT query, calls, rows FROM pg_stat_insights ORDER BY query COLLATE "C";

--
-- Don't reset anything if any of the parameter is NULL
--
SELECT pg_stat_insights_reset(NULL) IS NOT NULL AS t;
SELECT query, calls, rows FROM pg_stat_insights ORDER BY query COLLATE "C";
...
SELECT pg_stat_insights_reset(
	...
	(SELECT s.queryid FROM pg_stat_insights AS s
...
--
SELECT pg_stat_insights_reset(0,0,s.queryid) IS NOT NULL AS t
	FROM pg_stat_insights AS s WHERE s.query = 'SELECT $1 AS "ONE"';
SELECT query, calls, rows FROM pg_stat_insights ORDER BY query COLLATE "C";

--
-- remove query of a user (regress_stats_user1)
--
SELECT pg_stat_insights_reset(r.oid) IS NOT NULL AS t
...
-- reset all
--
SELECT pg_stat_insights_reset(0,0,0) IS NOT NULL AS t;
SELECT query, calls, rows FROM pg_stat_insights ORDER BY query COLLATE "C";
